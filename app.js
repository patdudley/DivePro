...
async function loadForecastData() {
  try {
    const [latest, tenDay, gradeGuide] = await Promise.all([
      fetchJson("/DivePro/model_outputs/latest_forecast.json"),
      fetchJson("/DivePro/model_outputs/forecast_10day.json"),
      fetchJson("/DivePro/model_outputs/diveprosd_grade_guidance.json"),
    ]);
    return {
      latest,
